...
from src.utils.utils import AdaptiveKLController, FixedKLController, RunningMoments
from src.models import SUPPORTED_ARCHITECTURES
...
class MotiongptPPOTrainerSepCritic(BaseTrainer):
    """
    ...
    def __init__(
        ...
        value_model: PreTrainedModelWrapper = None,
        policy_model: PreTrainedModelWrapper = None,
        ...
        if not isinstance(value_model, (SUPPORTED_ARCHITECTURES)):
            raise ValueError(
                f"model must be a PreTrainedModelWrapper, got {type(value_model)} - supported architectures are: {SUPPORTED_ARCHITECTURES}"
            )
        ...
        self.model_params = filter(lambda p: p.requires_grad, self.value_model.parameters())
        self.is_encoder_decoder = hasattr(self.value_model, "is_encoder_decoder")
        # self.is_peft_model = getattr(self.value_model, "is_peft_model", False)
        self.is_peft_model = config.is_peft_model
        config.is_encoder_decoder = self.is_encoder_decoder
        config.is_peft_model = self.is_peft_model

        # make a copy of the policy model as the value model
        ...
        self.optional_peft_ctx_policy = (
            self.accelerator.unwrap_model(self.policy_model).pretrained_model.disable_adapter
            if self.is_peft_model
            else nullcontext
        )

        self.optional_peft_ctx_value = (
            self.accelerator.unwrap_model(self.value_model).pretrained_model.disable_adapter
            if self.is_peft_model
            else nullcontext
        )
        ...
        if optimizer is None:
            self.optimizer = Adam(
                filter(lambda p: p.requires_grad, list(self.policy_model.parameters()) + list(self.value_model.parameters())),
        ...
        elif self.config.default_scheduler:
            num_train_steps = len(self.dataloader) * self.config.epochs
            self.lr_scheduler = self.create_scheduler(num_train_steps, self.optimizer)
    ...
    def create_scheduler(self, num_training_steps: int, optimizer: torch.optim.Optimizer = None):
        ...
        if self.lr_scheduler is None:
            warmup_steps = math.ceil(num_training_steps * 0.00)
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        if generate_ref_response:
            ref_model = self.policy_model if self.is_peft_model else self.ref_model
        if isinstance(query_tensor, List):
            ...
            if generate_ref_response:
                with self.optional_peft_ctx_policy() and self.optional_peft_ctx_value():
            ...
            response = self.accelerator.unwrap_model(self.policy_model).generate(
    ...
    def step(
        ...
        queries: List[torch.LongTensor],
        responses: List[torch.LongTensor],
        scores: List[torch.FloatTensor], # these are the rewards
        ...
        motion_kl_penalty = self.config.kl_penalty == "motion" # kl only considering motion tokens
        ...
        with torch.no_grad():
            all_logprobs, logits_or_none, values, masks = self.batched_forward_pass(
                ...
                return_logits=full_kl_penalty or motion_kl_penalty,
            )
            with self.optional_peft_ctx_policy() and self.optional_peft_ctx_value():
        ...
        with torch.no_grad():
            ...
            if full_kl_penalty:
                ...
                # new_vocab_ids = self.tokenizer.convert_tokens_to_ids(new_vocab)
                # # add pad and eos
                # # new_vocab_ids = [self.tokenizer.pad_token_id, self.tokenizer.eos_token_id] + new_vocab_ids
                # new_vocab_ids = torch.tensor(new_vocab_ids, device=self.current_device)
                # import pdb; pdb.set_trace()
                # logprob has shape [32, 32, 32615], we only select the last dimension that corresponds to the motion tokens
                # active_motion_logprobs = active_full_logprobs[:, :, new_vocab_ids]
                # ref_motion_logprobs = ref_full_logprobs[:, :, new_vocab_ids]
                active_motion_logprobs = active_full_logprobs[:, :, -515:]
                ref_motion_logprobs = ref_full_logprobs[:, :, -515:]

                rewards, non_score_reward, kls = self.compute_rewards(scores, active_motion_logprobs, ref_motion_logprobs, masks)
            else:
    ...
    def batched_forward_pass(
        self,
        policy_model: PreTrainedModelWrapper,
        ...
        for i in range(math.ceil(bs / fbs)):
            ...
            logits, loss = policy_model(**input_kwargs)[:2]
            values = value_model(**input_kwargs)
            ...
            masks[:, :-1] = attention_mask[:, 1:] # This is typically done in the context of sequence prediction tasks to ensure that the prediction for each position can only depend on the known outputs at positions less than the current one.
    ...
    def _kl_penalty(self, logprob: torch.FloatTensor, ref_logprob: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.config.kl_penalty == "full":
            ...
            # logprob shape is [32, 32615]
            return F.kl_div(ref_logprob, logprob, log_target=True, reduction="none").sum(-1)
        
        if self.config.kl_penalty == "motion":
            # only consider the following tokens: 
            return F.kl_div(ref_logprob, logprob, log_target=True, reduction="none").sum(-1)
    ...
    def compute_advantages(
        ...
        mask: torch.FloatTensor, # this mask is bs x T and is all ones except for the first index of every batch
    ):
        lastgaelam = 0
        advantages_reversed = []
        ...
        if self.config.whiten_rewards: # we do not normalize the rewards but we do normalize the advantages
        ...
        # this normalizes the advantages, this normalization happens at the minibatch level instead of the whole batch level
    ...
    def loss(
        ...
        vf_loss = 0.5 * masked_mean(torch.max(vf_losses1, vf_losses2), mask) # the 0.5 is to cancel out the 2 from the exponent
        ...
        if avg_ratio > self.config.ratio_threshold:
            ...
            # In Proximal Policy Optimization (PPO), the objective function is designed to prevent the policy from changing too much in one update. This is done by introducing a ratio, which is the new policy probability divided by the old policy probability.
            # If this ratio is outside of a certain threshold (usually [1-epsilon, 1+epsilon]), the objective function is clipped. This means that the policy update is limited and prevents overly large updates that could destabilize the learning process.
            # Skipping a batch when the ratio exceeds the threshold is a way to enforce this stability constraint. It ensures that the policy doesn't change too drastically from one update to the next, which could lead to harmful oscillations or divergence in the policy updates.


        # H(p) = - Σ [ p(x) * log(p(x)) ]
        entropy = masked_mean(entropy_from_logits(logits), mask)

        # KL(P || Q) = Σ [ exp(logP(x)) * (logP(x) - logQ(x)) ]
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        stats = {
            ...
            "objective/kl_distribution": kl_list,
            ...
            # "env/lr": self.lr_scheduler.get_last_lr()[0],
        }
    ...
    def log_stats(
        ...
        ref_rewards: List[torch.FloatTensor],
        ...
        if not isinstance(rewards, torch.Tensor):
            ...
            ref_rewards = ref_rewards.clone().detach().to(self.current_device)
        rewards = self.accelerator.gather(rewards).flatten()
        ref_rewards = self.accelerator.gather(ref_rewards).flatten()
        ...
        if self.accelerator.is_main_process:
            ...
            # torch.flatten(train_stats["policy/ratio"]).unsqueeze(0)
            logs["env/reward_distribution"] = wandb.Histogram(rewards.cpu().numpy().tolist())
            # logs["env/reward_dist"] = rewards.cpu().numpy()
            logs["env/ref_reward_mean"] = torch.mean(ref_rewards).cpu().numpy().item()
