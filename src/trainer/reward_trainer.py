...
from src.utils.utils import RewardDataCollatorWithPadding, compute_accuracy
...
class RewardTrainer(Trainer):
    ...
    def compute_loss(
        ...
        if "margin" in inputs:
            log_p_chosen = nn.functional.logsigmoid(rewards_chosen - rewards_rejected - inputs["margin"])
            log_p_rejected = nn.functional.logsigmoid(inputs["margin"] + rewards_rejected - rewards_chosen)
            loss = - (inputs["label"] * log_p_chosen + (1 - inputs["label"]) * log_p_rejected).mean()
            # log_p_chosen).mean() == nn.functional.logsigmoid(rewards_chosen - rewards_rejected - inputs["margin"]).mean()
            # loss = -nn.functional.logsigmoid(rewards_chosen - rewards_rejected - inputs["margin"]).mean()
        else:
            log_p_chosen = nn.functional.logsigmoid(rewards_chosen - rewards_rejected)
            log_p_rejected = nn.functional.logsigmoid(rewards_rejected - rewards_chosen)
            loss = (inputs["label"] * log_p_chosen + (1 - inputs["label"]) * log_p_rejected).mean()
